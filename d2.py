...
for y,x in product(range(img2.size[1]), range(img2.size[0])):
    ...
    # stego zoom4
    #if x%2==0 and y%2==0:
    #    new_p = pix4[x//2,y//2] * 255
    pix2[x,y] = new_p
    quant_error = old_p - new_p
    try:
        this_line[x+1] += 7 * quant_error / 16
        #pix2[x+1,y] += 7 * quant_error / 16
    except IndexError:
        pass
    try:
        next_line[x-1] += 3 * quant_error / 16
        #pix2[x-1,y+1] += 3 * quant_error / 16
    except IndexError:
        pass
    try:
        next_line[x] += 5 * quant_error / 16
        #pix2[x,y+1] += 5 * quant_error / 16
    except IndexError:
        pass
    try:
        next_line[x+1] += 1 * quant_error / 16
        #pix2[x+1,y+1] += 1 * quant_error / 16
    except IndexError:
        pass
for x2 in range(img2.size[0]):
